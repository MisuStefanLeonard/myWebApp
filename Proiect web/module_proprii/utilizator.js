...
class Utilizator{
    ...
    static tabel="webbd.utilizatori"
    static parolaCriptare="tehniciweb"; // de generat random
    static emailServer="test.webb1932@gmail.com";
    ...
    */
    ...
    salvareUtilizator(){
        ...
        let utiliz=this;
        let token1=parole.genereazaToken1(10);
        let nume_utiliz = utiliz.username + "-"
        let token2=nume_utiliz+parole.genereazaToken2(70);
        AccesBD.getInstanta(Utilizator.tipConexiune).insert({tabel:Utilizator.tabel,
            ...
            if(err)
                ...
                let userNameReversed = utiliz.username.split('').reverse().join('')
                console.log(userNameReversed)
                utiliz.trimiteMail("Te-ai inregistrat cu succes","Username-ul tau este "+utiliz.username,
            `<h1>Salut!</h1><p style='color:blue'>Username-ul tau este ${utiliz.username}.</p> <p><a href='http://${Utilizator.numeDomeniu}/${token1}/${userNameReversed}/${token2}'>Click aici pentru confirmare</a></p>`,
            )
        }
        });
    }
//xjxwhotvuuturmqm
    ...
    async trimiteMail(subiect, mesajText, mesajHtml, atasamente=[]){
        var transp= nodemailer.createTransport({
            ...
            auth:{//date login 
                ...
                pass:"ztpampykrrdjdgbo"
                //pass:"ztpa mpyk rrdj dgbo"
    ...
    static getUtilizDupaUsername (username,obparam, proceseazaUtiliz){
        if (!username) return null;
        let eroare=null;
        AccesBD.getInstanta(Utilizator.tipConexiune).select({tabel:"webbd.utilizatori",campuri:['*'],conditiiAnd:[`username='${username}'`]}, function (err, rezSelect){
            if(err){
