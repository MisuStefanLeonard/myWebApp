...
class AccesBD{
    ...
    initLocal(){
        this.client = new Client({database:"web",
        user:"stefan1",
        password:"admin12@",
        host:"localhost",
        port:5432});
    ...
    async updateAsync({ tabel = "", setari = {}, conditiiAnd = [] } = {}) {
        let setariString = Object.entries(setari)
        ...
        let comanda = `update ${tabel} set ${setariString} ${conditieWhere}`;
        console.error("updateAsync:", comanda);
        try {
            let rez = await this.client.query(comanda);
    ...
    // pentru a functiona cu async-ul
